...
class DrawFloatingMenu(FloatingMenu):
    ...
    def __init__(self, parent: QtWidgets.QWidget, view: "BeeGraphicsView"):
        ...
        # Кнопка очистки
        self.add_separator()
        trash_icon = QtGui.QIcon(str(icons_path.joinpath('trash-alt.svg')))
        self.clear_button = self.add_button(
            "",
            icon=trash_icon,
            callback=self.clear_drawing
        )
    ...
    def clear_drawing(self):
        """Очищает текущий рисунок."""
        if self.current_item:
            self.current_item.setPath(QtGui.QPainterPath())
